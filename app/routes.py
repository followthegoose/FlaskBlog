...
def index():
    form = PostForm()
    if form.validate_on_submit():                                   ####POST/REDIRECT/GET/
    ...
    return render_template('index.html', title='Active Gomez',
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Поздравляшки! Теперь ты зарегистрирован!')
        return redirect(url_for('login'))
...
'''Реквест на Восстановление пароля'''
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        user.set_password(form.password.data)
        db.session.commit()
        flash('Отлично! У вас новый пароль!')
        return redirect(url_for('login'))
